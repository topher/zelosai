...
const Search: React.FC = () => {
  // Find the Search route from the sidebar routes array
  const searchRoute = routes.find(route => route.label === 'Search');
  ...
  return (
    <div className={`container mx-auto flex flex-col justify-center h-full my-8 ${montserrat.className}`}>
      ...
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full flex-grow px-6">
        {searchRoute?.children?.map((child, index) => (
          <li
            key={index}
          ...
          >
            <Link href={child.href} className="block h-full">
              <div className="flex flex-col justify-center items-center h-full p-6">
                <div className="mb-4">
                  <Image
                    src={`/3dicons/${child.icon}/${child.icon}-dynamic-gradient.png`}
                    alt={child.label}
                    width={200}
                    height={200}
